...
| `04-message-mapping.lisp` | Map public typed records (including tool use/result readers) to a harness event schema | No |
| `05-control-handlers.lisp` | Permission/hook handlers; low-level vs session-start SDK MCP | Yes when CLI emits control traffic |
...
| `10-session-start-sdk-mcp.lisp` | Validated in-process SDK MCP catalog + source/permission options | No |
