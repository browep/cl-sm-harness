...
- [Permissions, hooks, and SDK MCP control messages](#permissions-hooks-and-sdk-mcp-control-messages)
...
|---|---|
| `:allowed-tools`, `:disallowed-tools` | Lists of strings. They map to Claude Code tool policy flags. |
...
public but its slot readers are not exported; `tool-use-block` and
`tool-result-block` are public types but their slot readers are not exported.
Treat the latter as typed routing signals in a public-only harness adapter.
`message-extra` preserves unknown top-level fields for decoded assistant/user
messages. A `system-message` retains its complete raw record in `data`.

## Permissions, hooks, and SDK MCP control messages

Only the persistent client services inbound control requests. Configure handlers
before `connect`; registration is frozen after connect. A handler runs
synchronously while the client consumes its current turn, and its result is
written on the same serialized client stream.
...
These APIs respond to inbound CLI control records. They do **not** register a
new external MCP server, create a general tool-loop abstraction, or make a
one-shot `query` answer permission requests. Use an interactive client whenever
the harness must answer control traffic.
...
| Options and local session plans | `make-agent-options` plus `agent-options-*` readers; `normalize-session-id`, `normalize-session-path`; `make-session-import-plan`, `make-session-mutation-plan` and their readers. |
| Messages | `message`, `user-message`, `assistant-message`, `message-extra`, `assistant-message-content`, `assistant-message-model`, `text-block`, `text-block-text`, `thinking-block`, `tool-use-block`, `tool-result-block`; decoding helpers. |
...
- [ ] Set an outer per-turn deadline for persistent clients and disconnect when
  it expires.
- [ ] Use explicit tool allow/deny policy plus control handlers; never return
...
queries, persistent stream-json clients, typed public records, inbound control
handlers, local session-store primitives, and fake transports. It does not
bundle the CLI, provide HTTP Messages API access, automatically mirror a full
conversation to a store, make `query` answer inbound control traffic, guarantee
subprocess-tree cleanup, or provide a persistent-client per-turn timeout. The
